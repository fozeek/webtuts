<?php 
	partial("header", array("header" => ($type=="bundle") ? $name : $type, "title" => null, "type" => $type,"name" => $name)); 
?>
...
<div id="search-node" style="float: right;cursor: pointer;position: relative;padding: 9px;border-left: 1px solid #E5E5E5;color: #232323;border-bottom: 1px solid #ccc;background: #f8f8f8;">
	...
	<div style="padding-right: 16px;background: url(<?= imageTheme("arrow-node.jpg") ?>) right center no-repeat;">
		<span id="node-show"><?= $name ?></span><span id="node-delete" style="display: none;color: #3F6EC2;margin-left: 5px;">x</span>
	</div>
	<div id="node-list" style="display: none;position: absolute;bottom: -3px;right: 3px;height: 0px;width: 0px;z-index: 50;">
		<div style="position: relative;width: 0px;height: 0px;">
			...
			<?php $bundles = Bundles::getBundle($name); foreach ($bundles["tables"] as $key => $value) : ?>	
				<a href="<?php echo Router::getUrl("content", "list", array("type" => $value)); ?>">
					<div style="padding: 5px;padding-left: 9px;border-bottom: 1px solid #E5E5E5;">
						<?= ucfirst($value) ?>
					</div>
				</a>
			<?php endforeach; ?>
